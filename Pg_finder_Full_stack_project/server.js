...
const uri = "mongodb+srv://tanyashetty2004:Tanya%401234@cluster0.sqytfxj.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0";
...
app.post('/registerowner', async (req, res) => {
    ...
    try {
        userData.password = md5(userData.password);  // Hash the password
...
app.post('/ownerlogin', async (req, res) => {
    ...
    try {
        ...
        const owner = await Owner.findOne({ username, password: hashedPassword });
        if (owner) {
...
app.post('/studentlogin', async (req, res) => {
    ...
    try {
        const hashedPassword = md5(password);  // Comparing the Stored hashed value to the entered password
        const student = await Student.findOne({ username, password: hashedPassword });
        if (student) {
            res.status(200).send('Login successful');
        } else {
            res.status(401).send('Invalid username or password');
